...
@section('page-header', 'Modifier : '.$filiere->name)
@section('content')
        ...
        <form action="{{ route('admin.filieres.update', $filiere) }}" method="POST">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $filiere->name) }}" required>
                    @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-6">
                    <label class="form-label">Code <span class="text-danger">*</span></label>
                    <input type="text" name="code" class="form-control @error('code') is-invalid @enderror" value="{{ old('code', $filiere->code) }}" required>
                ...
                <div class="col-md-6">
                    ...
                    <select name="coordinator_id" class="form-select">
                        ...
                        <optgroup label="Administrateurs">
                            ...
                            <option value="{{ $u->id }}" {{ old('coordinator_id', $filiere->coordinator_id)==$u->id ? 'selected':'' }}>{{ $u->name }}</option>
                            @endforeach
                        </optgroup>
                ...
                <div class="col-12">
                    ...
                    <textarea name="description" rows="4" class="form-control">{{ old('description', $filiere->description) }}</textarea>
                </div>
            </div>
            <div class="mt-4">
